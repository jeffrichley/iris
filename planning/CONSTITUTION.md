...
- MUST use Rich library for all terminal output
- MUST follow terminal color conventions
- MUST maintain dark theme (terminal default)
- MUST use Iris brand colors consistently

**Brand Colors (Terminal):**

```python
# Rich color definitions
IRIS_PRIMARY = "blue"          # Iris Blue
IRIS_BG_DARK = "black"        # Background
IRIS_SURFACE = "bright_black" # Cards/Panels
IRIS_SUCCESS = "green"        # Success states
IRIS_WARNING = "yellow"       # Warning states
IRIS_ERROR = "red"            # Error states
IRIS_TEXT = "white"           # Primary text
IRIS_TEXT_MUTED = "bright_black" # Secondary text
```

**Typography:**

- Use Rich's built-in terminal fonts
- Monospace for data tables
- Proportional for descriptions
...
- Terminal updates MUST be smooth and responsive
- Rich console notifications for async confirmations
...
async def save_task(task: Task):
    ...
    update_local_state(task)
    console.print("[yellow]Saving task...[/yellow]")

    try:
        await api.save(task)
        console.print("[green]Task saved successfully[/green]")
    except Exception as error:
        # Rollback optimistic update
        revert_local_state()
        console.print(f"[red]Failed to save task: {error}[/red]")
        console.print("[blue]Press R to retry[/blue]")
```
